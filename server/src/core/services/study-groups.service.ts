...
export class StudyGroupsService {
  ...
  async create(dto: CreateStudyGroupDto) {
    ...
    ///TODO: dto validation
    const tutor = await this.userRepository.findOneByOrFail({
      id: dto.tutor_id,
    });
    const students = await Promise.all(
      dto.students_ids.map((id) => this.userRepository.findOneByOrFail({ id })),
    );

    const group = this.studyGroupRepository.create({
      title: dto.title,
      tutor,
      students,
    });
